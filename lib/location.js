...
fns.getLocationByName = (requestData, done)=> {
    ...
    Joi.validate(requestData.data, validation.locByName, (err, validatedData) => {
        if (err) {
            return done(err);
        }


        db.getLocationsByName(validatedData.locationName, 'locations')
...
fns.getLocationsOfUser = (requestData, done) => {
    Joi.validate(requestData.data, validation.locOfUser, (err, validatedData) => {
        if (err) {
            return done(err);
        }

        db.getLocationsOfUser(validatedData.userId, 'locations')
