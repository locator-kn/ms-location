...
fns.getLocationByName = (requestData, done)=> {
        ...
        console.log(requestData.data.locationName);
   /* Joi.validate(requestData.data, validation.locByName, (err, validatedData) => {
        if (err) {
            return next(err);
        }

        if (!validatedData.data || !validatedData.data.locationName) {
            return done(new Error('missing location name'));
        }

        db.getLocationsByName(validatedData.data.locationName, 'locations')
