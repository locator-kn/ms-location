...
fns.getLocationById = (requestData, next) => {
    ...
    Joi.validate(requestData.data, validation.getLocationById, (err, data) => {
        ...
        if (err) {
            util.validationError(err, 'location get service', next);
        }

        db.genericById2(data.locationId, 'locations')
            .then(location => {

                if (!location) {
                    return next(null, {err: {msg: 'NOT_FOUND', detail: 'Location not found'}});
                }
                next(null, {data: location});
            })
            .catch(err => util.serviceError(err, 'location get service', next));
